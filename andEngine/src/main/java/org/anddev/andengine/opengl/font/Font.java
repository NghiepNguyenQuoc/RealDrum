...
public class Font {
    ...
    public Font(final ITexture pTexture, final Typeface pTypeface, final float pSize, final boolean pAntiAlias, final int pColor) {
        ...
        this.mLineHeight = (int) (float) Math.ceil(Math.abs(this.mFontMetrics.ascent) + Math.abs(this.mFontMetrics.descent)) + (PADDING * 2);
        this.mLineGap = (int) ((float) Math.ceil(this.mFontMetrics.leading));
    }
